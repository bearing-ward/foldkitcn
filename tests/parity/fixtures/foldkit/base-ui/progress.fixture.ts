...
const progressChildren = (
  ...
  config: CaseConfig,
  attributes: Progress.ProgressAttributes<never>,
  ...
  const maybeValue = config.includeValue
    ? [h.span([...attributes.value], [Progress.valueText(state)])]
    : []
...
const renderFoldkitProgress = (config: CaseConfig): FixtureSnapshot => {
  const h = html<never>()
  const state = Progress.progressState(config)
  ...
  return snapshotHtml(
    Progress.view<never>({
      ...
      toView: attributes =>
        h.div(
          [...attributes.root],
          [...progressChildren(h, config, attributes, state)],
        ),
    }),
  )
}
...
export const cases: ReadonlyArray<FixtureCase> = caseConfigs.map(config => ({
  id: config.id,
  snapshot: renderFoldkitProgress(config),
}))
