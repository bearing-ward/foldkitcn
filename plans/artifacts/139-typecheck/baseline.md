...
Result: exited nonzero. The raw compiler log is intentionally retained only in the executor's temporary directory, not in the repository.
...
The TS2859 reports occur almost entirely under `src/registry/**` tests, with a small number in `src/utils/cn.test.ts`. The non-complexity diagnostics span `scripts/report-docs-live-preview-gaps.ts`, root live-example application files, and base-ui/shadcn registry menu, select, tooltip, and date-picker files. They are contract errors to repair in bounded source or test lanes, not evidence that strict compiler options should be relaxed.
