...
Start with the pages users are most likely to inspect while comparing us to `ui.shadcn.com`:
...
`base-ui/button` was completed first because `/components/base-ui/button` had no live or static examples, even though the pinned Base UI origin had four demo paths.

## Work Pattern

For each item:

1. Add Foldkit-native examples in the registry source tree, normally under `src/registry/<namespace>/<slug>/examples.ts`.
2. Add example metadata to `registry-src/<namespace>/<slug>/item.json`.
3. Add or update `registry-src/<namespace>/<slug>/docs.md` if the page needs explanatory copy around the examples.
4. Run `bun run registry:build`.
5. Verify the generated `registry/docs/<namespace>/<slug>.json` contains examples with snippets, `previewStatus`, and `previewExportName`.
